...
static const char *JABC_NET_JS =
    ...
    "\n"
    "    _poll(revents) {\n"
    "      if (revents & (pol.ERR | pol.NVAL)) { this._fatal(\"socket error\"); return 0; }\n"
    "      if (revents & pol.OUT) { this._flush(); if (this._closed) return 0; }\n"
    "      if (revents & pol.IN) {\n"
    "        this._rb.reset();\n"
    "        let n = -1;\n"
    "        //  QJAB-012: ECONNRESET here would escape into pol.run and kill the loop.\n"
    "        try { n = net._recv(this.fd, this._rb.idle()); } catch (e) { this._fatal(e); return 0; }\n"
    ...
    "        }\n"
    "      } catch (e) { this._fatal(e); return; }   // QJAB-012: EPIPE on a vanished peer\n"
    "      if (this._wb.size === 0) {\n"
    "        this._wb.reset();\n"
    "        this.emit(\"drain\");\n"
    ...
    "    //  QJAB-012: a syscall on a dead peer throws inside the pol fd callback,\n"
    "    //  past every application try; Node answers with 'error' + a close, so do\n"
    "    //  that here — the pol.ERR revent above takes the same road.\n"
    "    _fatal(e) { this.emit(\"error\", (e && e.message) || String(e)); this._doClose(); }\n"
    "\n"
    "    write(data) {\n"
