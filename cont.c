...
static const char *JABC_CONT_JS =
    ...
    "    return v;\n"
    "  }\n"
    "  //  QJAB-008: a HASH table is slot-masked with (len-1) and probed one\n"
    "  //  16-slot line wide, so its capacity is rounded up to that shape here\n"
    "  //  (the _hash_* leaves reject any region that misses it).\n"
    "  const HASH_LINE = 16;\n"
    "  const pow2up = (n) => { let p = HASH_LINE; while (p < n) p *= 2; return p; };\n"
    "  const bytes = (family, slots) => {\n"
    "    if (isLog(family)) return slots;              // u8 bytes\n"
    "    const M = LANE[laneOf(family)];\n"
    "    const n = family.slice(0, 4) === \"HASH\" ? pow2up(slots) : slots;\n"
    "    return n * M.w * M.A.BYTES_PER_ELEMENT;\n"
    "  };\n"
    "\n"
    ...
    "  //  key), u64 and u32 (scalars).  Reopening a dir written by an older jab works:\n"
    ...
    "  //  QJAB-013: `big` is the lane's number domain and `A` its run view — the\n"
    "  //  64-bit lanes cross as BigInt, u32 as a plain Number.\n"
    "  const IDX = {\n"
    "    kv64:  { pair: true,  big: true,  A: BigUint64Array },\n"
    "    wh128: { pair: true,  big: true,  A: BigUint64Array },\n"
    "    u64:   { pair: false, big: true,  A: BigUint64Array },\n"
    "    u32:   { pair: false, big: false, A: Uint32Array    },\n"
    "  };\n"
    "  abc.index = (lane, opts) => {\n"
    "    const M = IDX[lane];\n"
    "    if (!M) throw \"abc.index: unsupported lane \" + lane + \" (u32 | u64 | kv64 | wh128)\";\n"
    "    opts = opts || {};\n"
    "    const dir = opts.dir;\n"
    ...
    "    //  QJAB-013: every needle crosses in the LANE's own number domain — a\n"
    "    //  BigInt() wrap on u32 would throw where the leaf wants a Number.\n"
    "    const N = M.big ? BigInt : (v) => v;\n"
    ...
    "    idx.put = M.pair\n"
    "      ? function (k, v) { _put(this._h, this.dir, this.ext, N(k), N(v)); return this; }\n"
    "      : function (v)    { _put(this._h, this.dir, this.ext, N(v)); return this; };\n"
    ...
    "    idx.get = function (k) { return _get(this._h, N(k)); };\n"
    "\n"
    "    //  ordered [lo, hi) scan through an in-frame cb; \"enough\"/false stops it.\n"
    "    idx.range = function (lo, hi, cb) { _range(this._h, N(lo), N(hi), cb); return this; };\n"
    "    //  prefix(p, lowBits) == range [p, p + 2^lowBits) — the keeper/spot block.\n"
    "    //  QJAB-013: the width is added in the lane's own domain, never mixed.\n"
    "    idx.prefix = function (p, lowBits, cb) {\n"
    "      const b = lowBits | 0;\n"
    "      if (!M.big) return this.range(p, p + Math.pow(2, b), cb);\n"
    "      const lo = BigInt(p);\n"
    "      return this.range(lo, lo + (1n << BigInt(b)), cb);\n"
    "    };\n"
    "\n"
    ...
    "    idx.run = function (i) {\n"
    "      const u = _run(this._h, i), A = M.A, w = A.BYTES_PER_ELEMENT;\n"
    "      return new A(u.buffer, u.byteOffset, (u.byteLength / w) | 0);\n"
    "    };\n"
