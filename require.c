...
static const char *JABC_REQUIRE_JS =
    ...
    "    return requireBase;\n"
    "  }\n"
    "  //  QJAB-009: normalize both candidates and demand the `<base>/` prefix —\n"
    "  //  raw ones let io.stat resolve `..` and mmap+eval a file above the base.\n"
    "  function resolveJsrc(spec) {\n"
    "    const base = pinBase(spec);\n"
    "    const under = base + \"/\";\n"
    "    for (const s of [spec, spec + \".js\"]) {\n"
    "      const c = normalize(under + s);\n"
    "      if (c.slice(0, under.length) !== under)\n"
    "        throw \"require: '\" + spec + \"' escapes the require base '\" + base +\n"
    "              \"'\";\n"
